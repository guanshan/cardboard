	...
	public void onCreate(Activity activity) {
		...
		this.nfcBroadcastReceiver = new BroadcastReceiver() {
			public void onReceive(Context context, Intent intent) {
				...
				NFCUtils.this.onNFCTagDetected((Tag) intent
						.getParcelableExtra("android.nfc.extra.TAG"));
			}
		};
		IntentFilter originalIntent = createNfcIntentFilter();
		originalIntent.addDataScheme("cardboard");

		IntentFilter shortenedIntent = createNfcIntentFilter();
		shortenedIntent.addDataScheme("http");
		shortenedIntent.addDataAuthority("goo.gl", null);

		IntentFilter explicitIntent = createNfcIntentFilter();
		explicitIntent.addDataScheme("http");
		explicitIntent.addDataAuthority("google.com", null);
		explicitIntent.addDataPath("/cardboard/cfg.*", 2);

		this.nfcIntentFilters = new IntentFilter[] { originalIntent,
				shortenedIntent, explicitIntent };
	}

	public void onResume(Activity activity) {
		activity.registerReceiver(this.nfcBroadcastReceiver,
