...
public class GyroscopeBiasEstimator {
	...
	public void reset() {
		...
		this.gyroBiasLowPass = new LowPassFilter(0.1500000059604645D);
	...
	public void processGyroscope(Vector3d gyro, long sensorTimestampNs) {
		...
		this.isGyroStatic
				.appendFrame(this.smoothedGyroDiff.length() < 0.00800000037997961D);
	...
	public void getGyroBias(Vector3d result) {
		if (this.gyroBiasLowPass.getNumSamples() < 30) {
			...
			double rampUpRatio = Math.min(1.0D,
					(this.gyroBiasLowPass.getNumSamples() - 30) / 100.0D);
